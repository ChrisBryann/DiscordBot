...
async def play(ctx, *url):
        ...
        while voice.is_playing() and not voice.is_paused():  # Checks if voice is playing
            await asyncio.sleep(1)  # While it's playing it sleeps for 1 second
        else:
            await asyncio.sleep(5)  # If it's not playing it waits 15 seconds
            while voice.is_playing() and not voice.is_paused():  # and checks once again if the bot is not playing
